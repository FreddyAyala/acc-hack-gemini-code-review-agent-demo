...
async def verify_api_key(
    settings: Settings = Depends(get_settings), api_key: str = Depends(api_key_header)
):
...
@app.get(
    "/api/users", response_model=models.UserList, dependencies=[Depends(verify_api_key)]
)
...
@app.get(
    "/api/users/{user_id}", response_model=models.User, dependencies=[Depends(verify_api_key)]
)
...
async def create_user(user: models.UserCreate, pool: asyncpg.Pool = Depends(get_pool)):
    try:
        async with pool.acquire() as connection:
